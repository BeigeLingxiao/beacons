...
// Methode which compute the BDCSVD for the int
    ...
    SVDBase <Matrix<int, Dynamic, Dynamic>> &
            BDCSVD<Matrix < int, Dynamic, Dynamic>
    >::compute(
    ...
    m_diagSize;
    i++) {
    this->m_singularValues.
    coeffRef(i) = 0;
}
if (this->m_computeFullU) this->
m_matrixU = Matrix<int, Dynamic, Dynamic>::Zero(rows(), rows());
if (this->m_computeFullV) this->
m_matrixV = Matrix<int, Dynamic, Dynamic>::Zero(cols(), cols());
this->
